...
mean_read_count <- 100  # Mean read count
...
# The mean must be adjusted by the size parameter to use rnbinom
...
ggplot(discoveries_df1, aes(x = Alpha, y = CombinedColumn, color = Method)) +
  ...
  labs(x = "Value", y = "Alpha", title = "Alpha vs. IHW/BH", color = "Type") +
